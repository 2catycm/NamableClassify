...
# AUTOGENERATED! DO NOT EDIT! File to edit: ../../src/notebooks/02data/infra.ipynb.

# %% auto 0
__all__ = ['dataset_name_choices', 'auto_augment_choices', 'simple_pixel_flatten_transform', 'cls_names_of',
           'ClassificationDataConfig', 'dataloader_to_arrays', 'ClassificationDataModule', 'sksplit_for_torch',
           'TorchVisionDataModule', 'CIFAR100DataModule', 'MNISTDataModule', 'CIFAR10DataModule',
           'FashionMNISTDataModule', 'QMNISTDataModule', 'KMNISTTDataModule']

# %% ../../src/notebooks/02data/infra.ipynb 5
from pathlib import Path
from ..help import data_path

# %% ../../src/notebooks/02data/infra.ipynb 6
...
# %% ../../src/notebooks/02data/infra.ipynb 7
...
# %% ../../src/notebooks/02data/infra.ipynb 8
# dataset_name_choices = [] # will be registered by `register_classification_data_module`
dataset_name_choices = cls_names_of([CIFAR100, CIFAR10, MNIST, FashionMNIST, QMNIST, KMNIST])
# auto_augment_choices = []
auto_augment_choices = cls_names_of([TrivialAugmentWide, AutoAugment, RandAugment, AugMix])

# def register_classification_data_module(cdm:'ClassificationDataModule'):
    

# %% ../../src/notebooks/02data/infra.ipynb 9
class ClassificationDataConfig(BaseModel):
    protocol: str = 'torch'
    dataset_root: Path = data_path
    # dataset_name: str = Field(default=..., 
    dataset_name: str = Field(default="CIFAR100", 
    ...
    choice_of_auto_augment: str = Field(default=TrivialAugmentWide.__name__, 
                                          ...
                                          choices = auto_augment_choices)
    # TODO 已经存在的 用户下载好的 data， 自动 ln -s 到 dataset_root 下面，但是数据源是来自于用户。
    # TODO  https://lightning.ai/docs/overview/prep-data/optimize-datasets-for-model-training-speed 优化数据集速度。
    # TODO s3存储服务。

# %% ../../src/notebooks/02data/infra.ipynb 10
...
# @memory.cache
def dataloader_to_arrays(dataloader: DataLoader) -> tuple:
    """Helper function to load all data from a dataloader into NumPy arrays"""

    # 预分配数组
    total_size = len(dataloader.dataset)  # 获取总数据量
    example = next(iter(dataloader))  # 获取一个样本
    input_shape = example[0].shape  # 获取输入数据的形状
    X_array = np.empty((total_size, *input_shape))  # 替换 input_shape 为 X_batch 的真实形状
    y_array = np.empty((total_size,))  # 根据 y 的形状进行调整

    index = 0
    for batch in dataloader:
        X_batch, y_batch = batch  # Assume each batch is (X, y)
        batch_size = X_batch.size(0)  # 获取当前 batch 的大小
        X_array[index:index+batch_size] = X_batch.cpu().numpy()  # 直接赋值
        y_array[index:index+batch_size] = y_batch.cpu().numpy()  # 直接赋值
        index += batch_size
...
class ClassificationDataModule(L.LightningDataModule):
    ...
    def __init__(self, **config:ClassificationDataConfig) -> None:
        ...
        # TODO faster loader with prefetching and fast collate
        # https://github.com/huggingface/pytorch-image-models/blob/main/timm/data/loader.py
        # TODO persistent_workers, HDF5 或者 TFRecords, drop_last=False, prefetch_factor
        # https://developer.aliyun.com/article/1599712
        # https://discuss.pytorch.org/t/how-does-prefetch-factor-really-work/171727/2
        
        # TODO NVIDIA DALI， GPU data augmentation
        # https://www.reddit.com/r/MachineLearning/comments/1bonupj/pytorch_dataloader_optimizations_d/?rdt=46715
        
        # TODO TFDS 的数据集 可以直接支持到 numpy格式，然后直接被 dataloader 加载
        # https://www.tensorflow.org/datasets/tfless_tfds?hl=zh-cn
    ...
    def setup(self, stage:str=None)->None:
        raise NotImplementedError("Please implement the `setup` method for your data module")
    ...
    # sklearn familar API
    # Property for training data
    @property
    def X_train(self) -> np.ndarray:
        if self._train_data is None:
            self._train_data = dataloader_to_arrays(self.train_dataloader())
    ...
    # Property for validation data
    @property
    def X_val(self) -> np.ndarray:
        if self._val_data is None:
            self._val_data = dataloader_to_arrays(self.val_dataloader())
        return self._val_data[0]
    
    @property
    def y_val(self) -> np.ndarray:
        if self._val_data is None:
            self._val_data = dataloader_to_arrays(self.val_dataloader())
        return self._val_data[1]
    
    # Property for test data
    @property
    def X_test(self) -> np.ndarray:
        if self._test_data is None:
            self._test_data = dataloader_to_arrays(self.test_dataloader())
...
# %% ../../src/notebooks/02data/infra.ipynb 12
...
# %% ../../src/notebooks/02data/infra.ipynb 13
...
# %% ../../src/notebooks/02data/infra.ipynb 14
...
# %% ../../src/notebooks/02data/infra.ipynb 15
# Simple Pixel Flatten Transform

simple_pixel_flatten_transform = Compose(
    # [v2.ToImage(), v2.ToDtype(torch.float32, scale=False), Lambda(lambda x: torch.flatten(x, start_dim=1))
    [v2.ToImage(), v2.ToDtype(torch.float32, scale=False), Lambda(lambda x: torch.flatten(x, start_dim=0))
]
)
@patch
def set_transform(self:ClassificationDataModule, train_transform, test_transform):
    self.hparams.train_transform = train_transform
    self.hparams.test_transform = test_transform

# %% ../../src/notebooks/02data/infra.ipynb 28
@patch
...
# %% ../../src/notebooks/02data/infra.ipynb 30
from fastcore.basics import patch
@patch
